<x-layouts.buylayout>
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-stone-950 font-nexo selection:bg-amber-500/30">
        
        <div class="lg:w-1/2 h-[60vh] lg:h-screen sticky top-0 bg-stone-900/20 flex items-center justify-center overflow-hidden border-r border-white/5">
            <div class="absolute w-[500px] h-[500px] bg-amber-600/10 rounded-full blur-[120px] animate-pulse"></div>
            
            <div class="absolute top-12 left-12 flex items-center space-x-4 opacity-50">
            ...
            <img src="{{ asset($okatia->image_url) }}" alt="{{ $okatia->model_name }}" 
                 class="relative z-10 w-3/4 max-h-[70%] object-contain drop-shadow-[0_40px_60px_rgba(0,0,0,0.9)] hover:scale-105 transition-transform duration-1000">
            
            <div class="absolute bottom-12 z-20 flex gap-2 p-1 bg-stone-900/50 backdrop-blur-md border border-white/5">
                @if($okatia->images && $okatia->images->isNotEmpty())
                    @foreach($okatia->images as $image)
                    <button class="w-14 h-14 bg-stone-950 border border-white/5 overflow-hidden group relative transition-all hover:border-amber-500/50">
        ...
        <div class="lg:w-1/2 p-8 md:p-16 lg:p-24 flex flex-col justify-center bg-stone-950 relative custom-scrollbar overflow-y-auto">
            
            <div class="absolute top-12 right-12 lg:right-auto lg:left-24">
                <a href="{{ route('collection') }}" class="group flex items-center space-x-4 text-[9px] uppercase tracking-editorial text-stone-500 hover:text-white transition-all">
            ...
            <header class="mb-16 animate-reveal opacity-0" style="animation-delay: 0.2s;">
                ...
                <div class="flex items-baseline space-x-6 mt-10">
                    ...
                    <span class="text-[9px] uppercase tracking-widest text-stone-600">Inclusive of global security transport</span>
                </div>
            </header>
